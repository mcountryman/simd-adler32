...
/// Returns an sse2 specialized adler32 update implementation.
#[allow(unused_variables)]
pub fn get() -> Option<Update> {
  let available = false;
  ...
  if available {
    Some(|a, b, data| unsafe { update(a, b, data) })
  } else {
    None
  }
}
...
/// # Safety
///
/// This function should only be called when `sse2` CPU instructions are
/// available.
